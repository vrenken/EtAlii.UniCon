...
namespace EtAlii.UniCon.Editor
{
    ...
    static class Coerce
    {
        ...
        public static bool DateTime(LogEventPropertyValue? value, out DateTimeOffset result)
        {
            if (value is ScalarValue sv)
            {
                if (sv.Value is string s)
        ...
        {
            if (value is ScalarValue sv && sv.Value is Func<LogEventPropertyValue, LogEventPropertyValue> pred)
            {
                predicate = pred;
                return true;
            }
